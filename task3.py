...
def morph_erode(img):
    ...
    bin_img=np.ones(shape=(3,3)).astype(int)
    ...
    for i in range(1,r-1):
        for j in range(1,c-1):
            z=[pad_img[i-1][j-1], pad_img[i-1][j], pad_img[i-1][j+1], pad_img[i][j-1], pad_img[i][j], pad_img[i][j+1], pad_img[i+1][j-1], pad_img[i+1][j], pad_img[i+1][j+1]]
            z=np.reshape(z,(3,3))
            az=np.multiply(z,bin_img)
            if np.prod(az)==0:
                t[i-1][j-1]=0
            else:
                t[i-1][j-1]=255
...
def morph_open(img):
    ...
    dilated_img=morph_dilate(eroded_img)
    
    
    open_img=dilated_img
    return open_img
...
def morph_close(img):
    ...
    eroded_img=morph_erode(dilated_img)
    
    close_img=eroded_img
    return close_img
...
def denoise(img):
    ...
    close_img=morph_close(open_img)
